...
{$Region ' 主动确认 ' }
  ...
  protected
    ...
    function getConnType : string;virtual;abstract;
  ...
  protected
    function getConnType : string;override;
  ...
  private
    BackupComfirmingPc : string;
  public
    constructor Create;
    procedure ClearBackupConfirm;
...
begin
    // 正在确认，不同时进行多次确认
  if PcID = BackupComfirmingPc then
  begin
    ConfirmFileInfo.Free;
    Exit;
  end;
...
procedure TFileConfirmThread.ClearBackupConfirm;
begin
  Lock.Enter;
  BackupFilePcConfirmList.OwnsObjects := True;
  BackupFilePcConfirmList.Clear;
...
begin
  ...
  begin
    ...
    BackupComfirmingPc := Result.PcID;
  end
  else
  begin
    Result := nil;
...
procedure TMyFileConfirm.ClearBackupConfirm;
begin
  if not IsRun then
    Exit;

  FileConfirmThread.ClearBackupConfirm;
end;
...
var
  ...
  ConnIp, ConnPort : string;
begin
  Result := False;
  ConnIp := MyNetPcInfoReadUtil.ReadIp( CloudPcConfirmInfo.PcID );
  ConnPort := MyNetPcInfoReadUtil.ReadPort( CloudPcConfirmInfo.PcID );
  if ( ConnIp = '' ) or ( ConnPort = '' ) then
    Exit;

    // 连接 目标 Pc
  MyTcpConn := TMyTcpConn.Create( TcpSocket );
  MyTcpConn.SetConnSocket( ConnIp, ConnPort );
  MyTcpConn.SetConnType( getConnType );
...
function TBackupFilePcConfirmHandle.getConnType: string;
begin
  Result := ConnType_ConfirmBackupFile;
end;
...
var
  CloudPath : string;
  CloudPcPath, CloudFilePath : string;
begin
  CloudPath := MyFilePath.getPath( MyCloudFileInfo.ReadBackupCloudPath );
  CloudPcPath := CloudPath + MyFilePath.getPath( CloudPcConfirmInfo.PcID );
  CloudFilePath := CloudPcPath + MyFilePath.getDownloadPath( FileInfo.FileName );
...
function TCloudFilePcConfirmHandle.getConnType: string;
begin
  Result := ConnType_ConnfirmCloudFile;
end;
...
constructor TAcceptFileConfirmHandle.Create;
begin
  SleepCount := 0;
end;
