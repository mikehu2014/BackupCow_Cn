...
uses Contnrs, ulkJson, TypInfo, SysUtils, Classes, Generics.Collections, SyncObjs, uDebug, UMyUtil;
...
{$Region ' 变化处理 ' }
  ...
  {$Region ' 数据线程 ' }
    ...
    // 处理 变化集合 线程
  TChangeHanldeThread = class( TThread )
  private
...
var
  ...
  MyManualChange : TMyChangeInfo; // 人工变化对象
...
begin
  ...
  begin
    ...
    if PropList[i]^.PropType^.Kind = tkClass then // 属性类
    begin
