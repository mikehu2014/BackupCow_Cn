...
type
  TFrameFilter = class(TFrame)
    ...
    procedure siLang_FrameFilterChangeLanguage(Sender: TObject);
  private
    IsInclude : Boolean;
    RootPathList : TStringList;
  private
...
begin
  ...
  begin
    ShowStr := siLang_FrameFilter.GetText( 'StrBigThan' );
    Caption := ShowStr + MySize.getFileSizeStr( LimitSpace );
...
begin
  ...
  begin
    Caption := siLang_FrameFilter.GetText( 'StrHideFile' );
...
begin
  ...
  with LvMask.Items.Insert(SpaceIndex) do
  begin
    ShowStr := siLang_FrameFilter.GetText( 'StrSmallThan' );
    Caption := ShowStr + MySize.getFileSizeStr( LimitSpace );
...
begin
  ...
  with LvMask.Items.Insert( InsertIndex ) do
  begin
    Caption := siLang_FrameFilter.GetText( 'StrSystemFile' );
    ImageIndex := MyShellIconUtil.getSystemIcon;
    Data := LvMaskData;
  end;
end;
...
procedure TFrameFilter.RefreshLanguage;
begin
  siLang_FrameFilterChangeLanguage(nil);
end;
...
procedure TFrameFilter.siLang_FrameFilterChangeLanguage(Sender: TObject);
begin
  LvMask.Columns[0].Caption := siLang_FrameFilter.GetText( 'lvMask' );
end;
