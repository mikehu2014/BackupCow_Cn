...
type
  ...
  public   // UPNP 端口映射
    procedure AddMapping( LocalIp, LocalPort, InternetPort : string );
...
procedure TPortMapping.AddMapping( LocalIp, LocalPort, InternetPort : string );
var
  Protocol : string;
...
begin
  ...
  PortMappingDeion := 'BackupCow' + ' (' + LocalIp + ':' + LocalPort + ') ' + InternetPort + ' TCP';
  LeaseDuration := 0;
  ...
  body := '<?xml version="1.0"?>'#13#10
    ...
    + '</s:Body>'#13#10
    + '</s:Envelope>'#13#10;
  request := 'POST ' + controlurl + ' HTTP/1.1'#13#10
  ...
  try
    ...
    MyLog.Log( 'Rev Map Response : ' );
    MyLog.Logln( ResponseStr );
