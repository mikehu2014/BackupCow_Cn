...
{$Region ' 本地备份 ' }
  ...
  protected
    function IsSourceInclude( ItemPath : string ): Boolean; // 本地源路径 是否已包含
    function IsIncludeDes( ItemPath : string ): Boolean;  // 是否已包含 本地目标路径
    procedure RemoveChild( ItemPath : string );   // 删除路径的子路径
    procedure AddSourcePath( ItemPath : string );virtual;abstract; // 添加 源路径
  end;
  ...
  protected
    ...
    procedure AddSourcePath( ItemPath : string );override; // 添加 源路径
  ...
  public
    ...
    procedure Update;
  private
    function IsSourceInclude: Boolean; // 本地源路径 是否已包含
    function IsDesExist: Boolean; // 本地目标路径 是否存在
  private
    procedure AddSourceBackupPath; // 本地目标 备份的源
  end;
...
var // 拖动文件
  DragFile_IsFileTransferDes: Boolean = False; // 拖动文件
  DragFile_FileTransferDesStart: Boolean = True; // 是否开始拖动文件
...
var // 应用程序
  ...
  AppRun_IsHide: Boolean = False; // 系统自动运行
  App_IsFreeLimit: Boolean = False; // 试用版限制
  DefaultPage_MainForm: Integer = NbPage_LocalFileStatus; // 默认的显示页面
...
begin
  ...
  FilePath := ItemData.FilePath;
  if FileType = FileType_SourceFile then // 源文件
...
begin
  ...
  begin
    NodeData := Sender.GetNodeData(Node);
    if Column = VstLocalBackupDes_FilePath then // 文件
    begin
...
begin
  ...
  begin
    NodeData := Sender.GetNodeData(Node);
    if Column = VstLocalBackupSource_ItemPath then // 文件
      ImageIndex := NodeData.PathIcon
    else
...
begin
  ...
  begin
    NodeData := Sender.GetNodeData(Node);
    if Column = VstMyBackupDes_PcName then // 文件
    begin
      if NodeData.IsOnline then
        ImageIndex := CloudStatusIcon_Online
      else
...
begin
  ...
  begin
    NodeData := Sender.GetNodeData(Node);
    if Column = VstSearchDown_FileName then // 文件
...
begin
  ...
  begin
    ...
    if not DirectoryExists(FilePath) then // 只添加目录
      Continue;
...
begin
  ...
  if DesPathList.Count <= 1 then  // 不用选择路径
    Exit;
