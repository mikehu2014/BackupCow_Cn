...
type
  ...
  TfrmRegisterNew = class(TForm)
    ...
    procedure siLang_frmRegisterNewChangeLanguage(Sender: TObject);
  private
...
begin
  BindSort;
  BindToolBar;
  siLang_frmRegisterNewChangeLanguage( nil );
end;
...
procedure TfrmRegisterNew.siLang_frmRegisterNewChangeLanguage(Sender: TObject);
begin
  with lvComputer do
  begin
    Columns[ 0 ].Caption := siLang_frmRegisterNew.GetText( 'lvPcName' );
    Columns[ 1 ].Caption := siLang_frmRegisterNew.GetText( 'lvPcID' );
    Columns[ 2 ].Caption := siLang_frmRegisterNew.GetText( 'lvEdition' );
  end;
end;
  ...
  try
    ...
    begin
      ...
      if HttpResult = HttpResult_Error then
        MyMessageBox.ShowError( frmRegisterNew.Handle, frmRegisterNew.siLang_frmRegisterNew.GetText( 'OrderIDError' ) );
    end;
    ResultList.Free;
  except
  end;
...
begin
  ...
  if OrderID = '' then
  begin
    MyMessageHint.ShowError( frmRegisterNew.edtBatOrderNum.Handle, frmRegisterNew.siLang_frmRegisterNew.GetText( 'InputOrderID' ));
    Exit;
  end;
  ...
  begin
    MyMessageHint.ShowError( frmRegisterNew.lvComputer.Handle, frmRegisterNew.siLang_frmRegisterNew.GetText( 'SelectPc' ) );
    Exit;
  end;
...
begin
  ...
  begin
    MyMessageBox.ShowWarnning( frmRegisterNew.Handle, frmRegisterNew.siLang_frmRegisterNew.GetText( 'LicenseError' ) );
    Result := False;
  end;
end;
...
begin
  ...
  begin
    MyMessageBox.ShowWarnning( frmRegisterNew.Handle, frmRegisterNew.siLang_frmRegisterNew.GetText( 'InputLicense' ) );
    Exit;
  end;
...
begin
  ...
  if not IsActiveSuccess then
    ...
    // 显示 激活成功
  MyMessageBox.ShowOk( frmRegisterNew.Handle, frmRegisterNew.siLang_frmRegisterNew.GetText( 'RegisterComplete' ) );
  frmRegisterNew.Close;
end;
